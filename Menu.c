...
struct DEV_airline
{
	...
	int seat_num;
	char email[15];
	struct DEV_airline *following;
}
*begin, *stream;
struct DEV_airline *dummy;
void main()
{
...
void reserve(int x)
{
	...
	if (begin == NULL)
	{
        begin = stream = (struct DEV_airline*)malloc(sizeof(struct DEV_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct DEV_airline *)malloc(sizeof(struct DEV_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
...
void savefile()
{
	FILE *fpointer = fopen("DEV records", "w");
	if (!fpointer)
	{
	...
	printf("\n\n\t Details have been saved to a file (DEV records)");
	fclose(fpointer);
}
void display()
{
